...
var REGEXES = {
  ...
  error : [
    ...
    /(?:error|reason)\:\s*?(\d{3}(?:[\s\-]\d\.\d\.\d\d?)?) (.+)/mi,
    /(\d{3}[\s\-]\d\.\d\.\d\d?)/m,
    /(\d{3}\-\'\d\.\d\.\d\d?)/m,
  ],
  ...
  enhanced_types_no_such_recipient : [
    ...
    /((?:account|user|mailbox) (?:.+@.+ )?(?:is )?(?:terminated|disabled|inactive|not found|unavailable|invalid))/i
  ],
  ...
  enhanced_types_mailbox_full : [
    /(out of storage|(?:mailbox (?:is )?full)|(?:user (?:is )?over quota))/i
  ],

  error_code : [
    /(\d{3})[\s\-](\d\.\d\.\d)/,  // "550 5.4.1" or "550-5.4.1"
    /(\d{3})\-\'(\d\.\d\.\d)/, // "550-'5.4.1"
    /(\d{3})/ // "550"
  ]
};
