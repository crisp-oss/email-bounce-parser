...
var REGEXES = {
  ...
  command : [
    ...
    /error after ([A-Z\s]+)/m, // "LMTP error after RCPT TO"
    /while sending end of ([a-z\s]+)/m // "while sending end of data"
  ],
...
class Parser {
  ...
  __parseBounceDataError(str) {
    ...
    if (_match && _match.length > 1) {
      ...
      _label = this.__unquoteText(_match[2]);
