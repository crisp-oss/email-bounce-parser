...
var REGEXES = {
  ...
  error : [
    /said\: (\d+(?:[\s\-]\d\.\d\.\d\d?)?) (.+) \(in reply to/m,
    /LMTP error after (?:.+?)\:\<.+?\>\: (\d+(?:[\s\-]\d\.\d\.\d\d?)?) <.+?\>(.+)$/m,
    /Remote Server returned \'(\d+(?:[\s\-]\d\.\d\.\d\d?)?) (.+)\'/m
  ],
...
class Parser {
  ...
  parseBody(body) {
    ...
    _body = _body.replace(this.__regexes.line_break_single_indent_double, " ");

    // Replace double Line-break followed by single indent with double Line-break
    _body = _body.replace(this.__regexes.line_break_double_indent_single, "\n\n");

    // Replace single Line-break with regular space
    _body = _body.replace(/(?<!\n)\n(?!\n)/gm, " ");

    // Replace long indent with double Line-break
    _body = _body.replace(this.__regexes.indent_long, "\n\n");
    ...
    if (_match && _match.length > 2) {
      return {
        ...
        intro   : trimString(_match[0] + _match[1]),
        error   : trimString(_match[2])
      };
    }
  ...
  __parseBounceDataError(str) {
    ...
    var _match = loopRegexes(this.__regexes.error, str);
